...
nm_fftw3_double(int argc, VALUE *argv, VALUE self)
  /* to be called by nm_fftw3 */
#else
nm_fftw3(int argc, VALUE *argv, VALUE self)
  /* to be called directly */
#endif
{
  VALUE val, vdir;
  struct {NMATRIX *a1, *a2};
  ...
  if (argc < 2) {
    rb_raise(rb_eArgError, "Usage: fftw(nmatrix, direction [,dim0,dim1,...])");
  }
  val  = argv[0];
  vdir = argv[1];
  ...
  v1 = nm_cast_object(val, nm_DCOMPLEX);
  GetNMatrix(v1,a1);
  v2 = nm_make_object( nm_DCOMPLEX, a1->rank, a1->shape, CLASS_OF(v1) );
  GetNMatrix(v2,a2);
  ...
  if (argc==2) {
    ...
    for(j=0; j<a2->rank; j++) bucket[j] = 0; /* initialize each element */
...
/* sourse code generation of nm_fftw3_float:
  Copy nm_fftw3_double, and replace
     fftw3--> fftw3f
     DCOMPLEX --> SCOMPLEX
 */
static VALUE nm_fftw3_float(int argc, VALUE *argv, VALUE self)
{
  VALUE val, vdir;
  struct NMATRIX *a1, *a2;
  int i, dir, *shape, *bucket;
  fftw3f_plan p;
  fftw3f_complex *in, *out;
  volatile VALUE v1, v2;
  ...
  v1 = nm_cast_object(val, nm_SCOMPLEX);
  GetNMatrix(v1,a1);
  v2 = nm_make_object( nm_SCOMPLEX, a1->rank, a1->shape, CLASS_OF(v1) );
  GetNMatrix(v2,a2);
  ...
  in = (fftw3f_complex*)a1->ptr;
  out = (fftw3f_complex*)a2->ptr;

  if (argc==2) {
      /* apply FFT to all dimensions */
      p = fftw3f_plan_dft( a2->rank, shape, 
			 in, out, dir, FFTW_ESTIMATE );
  } else {
      /* apply FFT to selected dimensions (by using the Guru interface) */
      { /* introduce a new scope for additonal local variables */
    ...
    for (i=2;i<argc;i++) {
      dim = NUM2INT(argv[i]);
      if (dim<0) dim += a2->rank;  /* negative: count from the end */
      if (dim<0 || dim>=a2->rank) rb_raise(rb_eArgError, "dimension < 0 or >= rank");

     dimids[i-2] = a2->rank - 1 - dim;
      if ( i>2 && dimids[i-2] == dimids[i-3] ) {
        rb_raise(rb_eArgError, "redundant -- the same dimension is being repeated");
      }
    }

    /* bucket sort in increasing order  TODO move*/
    bucket = ALLOCA_N(int,a2->rank);
    ...
    for(j=0,i=0; j<a2->rank; j++) {
	  ...
	  p = fftw3f_plan_guru_dft( fft_rank, fft_dims, 
				  howmany_rank, howmany_dims,
				  in, out, dir, FFTW_ESTIMATE );
...
nm_fftw3(int argc, VALUE *argv, VALUE self)
  {
    ...
    val = argv[0];
    v1 = nm_to_nmatrix(val);
    GetNMatrix(v1,a1);

    if(a1->type <= nm_SFLOAT || a1->type == nm_SCOMPLEX ) {
        return( nm_fftw3_float(argc, argv, self) );
    } else {
        return( nm_fftw3_double(argc, argv, self) );
    }
  }
 ...
 Init_fftw3()
{
  ...
  rb_define_module_function(mFFTW3, "fft_raw", nm_fftw3, -1);
}
